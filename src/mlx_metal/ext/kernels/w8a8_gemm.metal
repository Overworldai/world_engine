...
//
// Uses MLX's optimized NAXTile/BaseNAXFrag/tile_matmad_nax directly.
// Both A and B staged through threadgroup with coalesced 128-bit loads.

#include <metal_stdlib>
#include <metal_simdgroup>

// MLX Steel NAX abstractions — same code path as mx.matmul
    ...
    uint3 tgid, uint sgid, uint lane)
{
    ...
    const short sgp_sn = sg_valid ? short(metal::min(int(N) - int(n_base + tn), int(_SN))) : 0;

    // Accumulators — using MLX's NAXTile with int (int32) element type
    ...
    for (uint k_base = 0; k_base < K; k_base += _BK) {
        ...
        // Cooperative coalesced load A [BM, BK] → threadgroup (128-bit / 16 bytes per thread)
        ...
        if (sg_valid) {
            ...
            for (short kk = 0; kk < _BK; kk += _SK) {
                // Load A fragments from threadgroup using MLX's NAXTile::load
                NAXTile<int8_t, _TM, _TK> Atile;
                Atile.template load<int8_t, _LDA_TG, 1>(
                    As + tm * _LDA_TG + kk);

                // Load B fragments from threadgroup
                NAXTile<int8_t, _TN, _TK> Btile;
                Btile.template load<int8_t, _LDB_TG, 1>(
                    Bs + tn * _LDB_TG + kk);

                // MMA using MLX's tile_matmad_nax — int8×int8→int32
                tile_matmad_nax(
                    Dtile,
...
// Both A+B staged as int8 (2x denser than fp16)
// Budget: BM*(BK+16) + BN*(BK+16) <= 32768
